...
if (isDeno) {
  ...
  expect = (value: any) => ({
    ...
    toThrow: (matcher?: RegExp) =>
      assertThrows(() => value(), Error, matcher?.source),
  });
} else {
...
describe("switchCase utility", () => {
  ...
  describe("discriminated union handling", () => {
    ...
    // --- Chainable discriminated union builder tests ---
    it("should handle discriminated unions with chainable builder syntax", () => {
      const square = { kind: "square", size: 7 } as Shape;
      const result = switchCase(square, "kind")
        .case("circle", (s) => Math.PI * s.radius ** 2)
        .case("square", (s) => s.size ** 2)
        .case("rectangle", (s) => (s as Rectangle).width * s.height)
        .run();

      expect(result).toBe(49);
    });

    it("should use default handler with chainable builder syntax", () => {
      // @ts-expect-error - Intentionally using a non-existent kind for testing
      const shape: Shape = { kind: "triangle", base: 10, height: 5 };
      const result = switchCase(shape, "kind")
        ...
        .default((s) => `Unknown shape: ${(s as any).kind}`)
        .run();

      expect(result).toBe("Unknown shape: triangle");
    });

    it("should throw error when no chainable builder case matches and no default is provided", () => {
      // @ts-expect-error - Intentionally using a non-existent kind for testing
      const shape: Shape = { kind: "triangle", base: 10, height: 5 };
      expect(() => {
        switchCase(shape, "kind")
  ...
  describe("chainable builder pattern", () => {
    ...
    it("should not throw when exhaustive has a default handler", () => {
      const result = switchCase(
        "rejected" as "pending" | "fulfilled" | "rejected",
      )
        .case("pending", "Loading...")
        .case("fulfilled", "Success!")
  ...
  describe("assertUnreachable utility", () => {
    it("should throw an error with the provided value", () => {
      expect(() => assertUnreachable("something" as never)).toThrow(
        /Unreachable case: something/,
      );
    });
  });
